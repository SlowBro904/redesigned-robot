...
mywifi = sta_ap()
assert mywifi.ssid == config['WIFI_SSID'], "WIFI_SSID"
good("WIFI_SSID")
...
assert mywifi.sec_type2int('WPA') == WLAN.WPA2, "sec_type2int()"
good("sec_type2int()")
assert 'WPA' == mywifi.sec_type2str(WLAN.WPA2), "sec_type2str()"
good("sec_type2str()")
assert mywifi.ant2int(WLAN.EXT_ANT) == 'External', "ant2int()"
good("ant2int()")
assert mywifi.mode2int('STA_AP') == WLAN.STA_AP, "mode2int()"
good("mode2int()")
assert mywifi.ifconfig == ('', '', '', ''), "Non-connected ifconfig()"
good("Non-connected ifconfig()")
assert mywifi.connect() is True, "connect()"
good("connect()")
assert mywifi.disconnect() is True, "disconnect()"
good("disconnect()")
mywifi.connect()
assert mywifi.isconnected() is True, "isconnected()"
good("isconnected()")
assert mywifi.ifconfig() == (ip, subnet_mask, gateway, DNS_server), "ifconfig()"
good("ifconfig()")
