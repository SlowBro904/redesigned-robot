class WIFI(object):
  ...
  def security_type2str(self, security_type):
    ...
    result = 'None'
    if    security_type == self.WLAN.WPA2:  result == 'WPA2'
    elif  security_type == self.WLAN.WPA:   result == 'WPA'
    elif  security_type == self.WLAN.WEP:   result == 'WEP'
  ...
  def security_type2int(self, security_type):
    ...
    if    security_type == 'WPA2':  result = self.WLAN.WPA2
    elif  security_type == 'WPA':   result = self.WLAN.WPA
    elif  security_type == 'WEP':   result = self.WLAN.WEP
  ...
  def connect(self):
    ...
    try: self.wlan.connect(self.SSID, auth=(security_type_int), self.password), timeout=10000)
    except: pass
  ...
  def ifconfig(self, ip = '', subnet_mask = '', gateway = '', DNS_server = ''):
    ...
    if ip and subnet_mask: # We don't always need gateway and DNS server.
      ...
      try: self.wlan.ifconfig(config=(ip, subnet_mask, gateway, DNS_server))
      except: pass
    
    try: return self.wlan.ifconfig()
    except: return ('','','','') # TODO Is this redundant
