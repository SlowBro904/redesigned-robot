...
class DS3231(object):
  ...
  def tempc_get(self):
    """Return Celsius temp in float"""
    temp = self.read(0x11, 2)
    temp = ((temp[0] << 8) | temp[1]) >> 6
    if temp&0x200: temp|=-1^0x3ff # Sign extend
    return temp/4.0

  def tempf_get(self):
    """Return Fahrenheit temp in a float"""
    return self.tempc_get()*1.8+32
...
if __name__=='__main__':
  ...
  if cmd=='start' or cmd=='reload' or cmd=='restart':
    ...
    print "%.2fc / %.2ff" % (clock.tempc_get(), clock.tempf_get())
